...
final class AdminKernel
{
    ...
    public function install(Request $request): Response
    {
        ...
        } elseif (str_starts_with($dataRoot, dirname(__DIR__, 2) . '/public')) {
            $errors[] = 'Data root must NOT be inside the public/ directory.';
        }
        if (!preg_match('/^mysql:host=[^;]+;dbname=[a-zA-Z0-9_]+/', $dsn)) {
            $errors[] = 'Database DSN must look like mysql:host=127.0.0.1;dbname=minis3 (MySQL/MariaDB only).';
        }
        ...
        try {
            ...
            $config = [
                ...
                'log_file' => rtrim($dataRoot, '/') . '/mini-s3.log',
    ...
    {
        $dataRootDefault = dirname(__DIR__, 2) . '/data';
        $checks = [];
        ...
        $checks['config_absent'] = [
            'ok' => !is_file(dirname(__DIR__, 2) . '/config.php'),
            'detail' => is_file(dirname(__DIR__, 2) . '/config.php') ? 'config.php already exists' : 'ready to write config.php',
        ];
        $checks['https'] = [
    ...
    {
        $root = (string) (minis3_config()['data_root'] ?? '');
    ...
    private function writeConfig(array $config): void
    {
        ...
        $content = "<?php\n\n// mini-s3 configuration — generated by the web installer.\n"
